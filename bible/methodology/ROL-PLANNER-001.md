...
domain: process
scope: task
trigger: When PBK-PROC-ORCHESTRATOR-001 dispatches a planner subagent after exploration completes.
statement: 'Subagent role template for planning: consumes exploration results, writes plan.md (four sections) and capabilities.md; writes no code. Fresh session, no master inheritance.'
rationale: Isolating planning in its own session forces a complete, reviewable plan artifact before any code is written, and keeps the planner from drifting into implementation.
tags:
- planning
- process
- subagent
...
Non-retrievable. Writes plan.md + capabilities.md; dispatched after exploration.
