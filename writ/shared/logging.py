...
# Default log root: `$HOME/.cache/writ/logs`, one per user. NOT derived from this
# module's own `__file__`: the daemon (repo checkout), the hooks (plugin cache) and
# the agent's shell are three installs of this code on one machine, and an
# install-relative default split every stream across three trees, the same split
# that lost gate approvals in the session store (see writ/session/cache.py). Same
# derivation as `_DEFAULT_CACHE_DIR` there. Evaluated once at import; the
# `WRIT_LOG_ROOT` env override still wins (checked first in `log_root`).
_DEFAULT_LOG_ROOT = Path.home() / ".cache" / "writ" / "logs"


def log_root() -> Path:
    """The central Writ-owned log root: `WRIT_LOG_ROOT` env or `$HOME/.cache/writ/logs`."""
