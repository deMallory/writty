...
# Mirrors writ/session/cache.py: WRIT_CACHE_DIR wins, else $HOME/.cache/writ/session,
# one store per user. NOT derived from this file's own location: the daemon (repo
# checkout), the hooks (plugin cache) and the agent's shell are three installs of this
# code on one machine, and an install-relative default gave each its own store, so the
# approval hook read a store with no mode and never posted the advance (2026-09-14).
#
...
writ_session_cache_dir() {
    ...
    printf '%s' "$HOME/.cache/writ/session"
}
