...
async def pipeline_db():
    ...
    # dump. Best-effort -- if it's missing or fails, downstream tests will
    # hit empty Neo4j (the original isolation bug); we surface that via
    # stderr but do not raise. Previously hardcoded `cwd` to
    # `~/.claude/skills/writ` (a different, private repo checkout) --
    # fixed to resolve relative to THIS repo.
    try:
        subprocess.run(
