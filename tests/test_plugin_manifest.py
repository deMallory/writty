...
class TestMarketplaceManifestExists:
    ...
    def test_it_names_the_marketplace_and_an_owner(self):
        """Fork policy: the marketplace is named `writty` (install writty@writty)."""
        mkt = _marketplace_manifest()
        assert mkt.get("name") == "writty", (
            "the fork installs via `claude plugin install writty@writty`, so the marketplace "
            f"name must be 'writty', got {mkt.get('name')!r}"
        )
