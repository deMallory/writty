...
**State the daemon does not own:** session caches are files under `~/.cache/writ/session` (one store per user, never install-relative; hooks and CLI read them directly when the daemon is down); route modules read `server._pipeline` / `server._db` as live attributes (the monkeypatch seam; never `from`-import them).
