...
| **Hooks + session state machine** | 45 bash hooks intercept the Claude Code tool lifecycle (49 registrations, 12 events, `hooks/hooks.json`); a Python package owns mode/phase/budget/gate state per session. | `hooks/`, `writ/session/` |
