...
- **`WRIT_CACHE_DIR`** defaults to a fresh `mkdtemp`: without it, subprocess tests would pollute the real `~/.cache/writ/session` (the production default moved off `/tmp`, then off the install dir).
