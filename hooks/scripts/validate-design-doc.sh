...
CONTENT=$(echo "$HOOK_ENVELOPE" | python3 -c "import sys,json; print((json.load(sys.stdin).get('tool_input') or {}).get('content',''))")
[ -z "$CONTENT" ] && exit 0

DENY=$(WRIT_DESIGN_CONTENT="$CONTENT" python3 <<'PY'
import os, re
content = os.environ.get("WRIT_DESIGN_CONTENT", "")
