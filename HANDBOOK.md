...
Every load-bearing number below is anchored to a file, a command you can run, or a dated measurement. Where this handbook and the code disagree, the code wins. Counts were taken from the shipped corpus dump and source tree on 2026-08-14.
...
**Shipped corpus** (`writ-corpus.cypher`, 2026-08-14: **468 nodes / 1,268 edges**; run the census below for your live graph):

| Node type | Count | Retrieval path |
|---|---:|---|
| Rule | 288 (32 mandatory) | ranked pipeline; mandatory via `/always-on` only |
...
| Skill / Playbook | 16 / 15 | companion channel + ranked pipeline |
| AntiPattern / Technique | 14 / 14 | companion channel + ranked pipeline |
| SubagentRole | 5 | `/subagent-role/{name}` only |
...
Everything else is advisory or observational: the RAG injectors (`writ-rag-inject`, `writ-read-rag`, `writ-posttool-rag`, `writ-pre-write-dispatch`'s allow path), post-write validators (`validate-file`, `validate-rules`, `validate-handoff`), methodology pushes (`inject-tier-workflow`, `writ-bible-authoring-push`), lifecycle (`session-start-bootstrap`, `writ-precompact`, `writ-postcompact`, `writ-session-end`, `writ-pressure-audit`, `writ-cwd-changed`), sub-agent lifecycle (`writ-subagent-start/stop`, whose stop side also runs the transcript tripwire: it records a critical event when a sub-agent turn shows the harness's queued-input misdelivery shape, and deliberately blocks nothing), evidence capture (`writ-web-capture`), the observe-by-default read-junk gate, and the opt-in blackbox capture. One nuance: `validate-rules.sh` is advisory per write, but at the plan boundary a confirmed violation invalidates the phase-a gate, which the *next* run enforces with a blocking exit. A second nuance: `writ-postcompact.sh` is observational in the strictest sense, because it emits nothing at all. Claude Code gives `PostCompact` no model-visible channel (the event is in neither `STDOUT_TO_MODEL_EVENTS` nor `ADDITIONAL_CONTEXT_EVENTS` in `writ/shared/delivery.py`, and CC's hook-output validator rejects a `PostCompact` `hookSpecificOutput` reply outright). The hook therefore resets the post-compaction state and sets `post_compact_pending`, and `writ-rag-inject.sh` emits the re-orientation on the next `UserPromptSubmit`, which is a confirmed channel.

**Patterns every hook follows:** one-spawn envelope parsing (`load_hook_env`), daemon-first with subprocess fallback (`_writ_session`, curl timeouts in the 100-500 ms range), fail-open when the daemon is down, an EXIT trap that logs every hook execution on every exit path, and delivery through `additionalContext` (bare stdout reaches the model on `UserPromptSubmit`, `UserPromptExpansion` and `SessionStart` only; on Pre/PostToolUse it goes to the Claude Code debug log and nowhere else). `PreCompact` and `PostCompact` have no model channel at all, neither stdout nor `additionalContext`, so a hook on either writes state and defers its message to the next `UserPromptSubmit`. Blocking Stop hooks use stderr plus a non-zero exit and check `stop_hook_active`, because a Stop hook's `additionalContext` is treated as a turn-continue and loops.
...
Per-stage and scale figures live in `SCALE_BENCHMARK_RESULTS.md` (dated measurements; both the synthetic curve and the live measurement below are 2026-08-01, at the 287-rule corpus of that date, on the machine described in that file's "Measurement environment" section). The shipped corpus is 288 rules as of 2026-08-14, so the table below is the 2026-08-01 measurement, not a current reading:

| Stage | p95 @ 287 rules, 2026-08-01 (live) | p95 @ 10K rules (synthetic) |
|---|---:|---:|
...
Retrieval quality gates are *floors*, not targets (MRR@5 >= 0.45 on the 47 ambiguous queries, hit rate >= 0.75 on all 193). The published measurement of 0.5681 / 0.7824 was taken on 2026-08-01 against the 287-rule corpus of that date and has not been re-measured since. The floors themselves re-ran green on 2026-08-14 against today's 288-rule corpus (`make bench`, 17 of 17). The floors were deliberately walked down as the corpus grew 4x; the history and rationale live in `tests/fixtures/regression_floors.py`.
